function [partedData, gatingProbability, classProbability, errorFlag] = partition( data, mu, sigma, pi_k)
    ...
    classProbability = pi_k .* classProbability;
    ...
    for k = 1:K
        ...
        kInds = clusterInds(:,k);
        C_kk{k} = [x(kInds,:), y(kInds)];
        R_kk{k} = R( kInds, kInds);
    end
